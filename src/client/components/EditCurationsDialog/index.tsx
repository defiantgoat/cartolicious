...
import { Dialog, IconButton, List, ListItem, TextField } from "@material-ui/core";
import { Curation, ReduxStateConfigProps } from "../../interfaces";
import { toggleCurationsDialog } from "../../actions";
import { Delete, EditRounded, SaveRounded, Visibility } from "@material-ui/icons";
...
const CurationItem: React.FC<{ curation: Curation }> = ({ curation }) => {
  ...
  const handleUpdateCuration = async () => {
    const {status, data, errors} = await updateCuration({id, name: curationName});
    if (status === 200) {
  ...
  return (
    !deleted ?
    <ListItem key={`curation-${id}`} style={{ borderBottom: "1px solid #ccc", display: "flex" }}>
      <div style={{ display: "flex", flex: 1 }}>
        <div style={{ flexGrow: 1, display: "flex", alignItems: "center" }}>
          {editName ?
            <TextField disabled={!editName} variant="outlined" defaultValue={curationName} style={{ flex: 1 }} onChange={handleCurationName}></TextField>
            : curationName
          }
        ...
        <div style={{ flexBasis: "33%", display: "flex", justifyContent: "flex-end" }}>
          {!editName ? <>
            <IconButton onClick={handleLoadCuration}><Visibility /></IconButton>
            <IconButton onClick={() => setEditName(true)}><EditRounded /></IconButton>
            <IconButton onClick={handleDeleteCuration}><Delete /></IconButton>
          </>
            : <>
              <IconButton onClick={handleUpdateCuration}><SaveRounded /></IconButton>
            </>}

        </div>
      </div>
    </ListItem>
    : <ListItem key={`curation-${id}`} style={{ borderBottom: "1px solid #ccc", display: "flex" }}><p>deleted</p></ListItem>
  )
}

const EditCurationsDialog: React.FC = ({ children }) => {
  ...
  return <Dialog fullWidth={true} maxWidth="md" open={open} onClose={handleClose}><div style={{ padding: "2rem" }}>
    <List>
      {createCurationList()}
    </List>
  </div></Dialog>;
